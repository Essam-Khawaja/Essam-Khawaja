# Hi, I'm Essam 👋

**Computer Science Student @ University of Calgary**

I build things. AI-powered applications, Kaggle models, hackathon projects—if it involves code and solves a problem, I'm interested.

## 🚀 Featured Projects

**[SwiftApply](https://github.com/Essam-Khawaja/swiftapply)** - Chrome Extension using Gemini Nano  
Cuts job application time from 50 minutes to 3 minutes with AI-powered analysis and cover letter generation. Won Google Chrome Built-in AI Challenge 2025.

**[Kaggle Competitions](https://github.com/Essam-Khawaja/Kaggle-Competitions)** - ML Competition Portfolio  
ROC AUC 0.961 on Bank Subscription Prediction. Active participant exploring different ML techniques and datasets.

**[AI Dungeon Master](https://github.com/Essam-Khawaja/Masse)** - Agentic AI Web App  
Built for ODSC Hackathon using Gemini API. Python backend + Next.js frontend with pixel art theme.

## 🛠 Tech I Use

**ML/AI**: Python, TensorFlow, PyTorch, Scikit-learn, Pandas, NumPy  
**Web**: TypeScript, React, Next.js, Tailwind CSS  
**Tools**: Git, Jupyter, Chrome Extensions (WXT)

## 🎯 Currently

- Building ML projects that actually ship
- Competing on Kaggle
- Exploring on-device AI and LLM applications
- Looking for Summer 2025 ML Engineering internships

## 📫 Connect

[LinkedIn](https://www.linkedin.com/in/syed-essam) • [Kaggle](https://www.kaggle.com/essamkhawaja) • syedessam345@gmail.com

---

![GitHub stats](https://github-readme-stats.vercel.app/api?username=Essam-Khawaja&show_icons=true&theme=dark)
